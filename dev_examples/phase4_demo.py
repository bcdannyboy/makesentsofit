    ...
    for day in range(7):
        for hour in [9, 12, 15, 18, 21]:  # Peak hours
            for author, (primary_sentiment, ratio, base_engagement) in authors.items():
                ...
                # Create post
                post = Post(
                    id=f"{day}_{hour}_{author}",
                    platform='twitter' if hour % 2 == 0 else 'reddit',
                    author=author,
                    author_id=f"id_{author}",
                    content=f"Post about topic at day {day} hour {hour} with sentiment {sentiment}",
                    title=f"Title {day}-{hour}" if hour % 2 == 1 else None,
                    ...
                    query='demo_query',
                    metadata={
                        'hashtags': ['demo', 'test'] if hour in [12, 18] else ['demo'],
                        'mentions': ['user1', 'user2'] if author == 'positive_influencer' else []
                    }
                )
